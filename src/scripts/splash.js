...
window.addEventListener("load", () => {
  ...
  if (!splash) return; // Por si no existe

  // Añade clase para animar el fade out tras 1.5s
  setTimeout(() => {
    splash.classList.add("fade-out");

    // Tras 0.5s (duración del fade), elimina splash y su CSS
    setTimeout(() => {
      splash.remove();

      const splashCss = document.getElementById("splash-css");
